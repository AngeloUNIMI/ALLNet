...
import pickle
import PIL
from lbcnn_pytorch.lbcnn_model import Lbcnn
import time
...
import util
import functions
from modelGeno.vgg16_bn_Mahdi import vgg16_bn_Mahdi_lbcnn
from modelGeno.resnet_geno import resnet18_lbcnn
from modelGeno.resnet_geno import resnet34_lbcnn
...
if __name__ == '__main__':
    ...
    # ------------------------------------------------------------------- db info
    dirWorkspaceOrig = 'D:/Workspace/DB HTT - Public (orig) (Mahdi)/'
    dirWorkspaceTest = 'D:/Workspace/DB HTT - Public (test) (Mahdi)/'
    # dirWorkspaceOrig = 'E:/Angelo/Workspace/DB HTT - Public (orig) (Mahdi)/'
    # dirWorkspaceTest = 'E:/Angelo/Workspace/DB HTT - Public (test) (Mahdi)/'
    dbName = 'ADP'
    ...
    dirDbOrig, dirDbTest, dirOutDatastore, dirOutTrainTest = dirsADP.computeDirs()
    ...
    # ------------------------------------------------------------------- transform db
    # dLabel = util.dbToDataStore(dirDbOrig, dirOutDatastore, extOrig, extNew, log)
    dLabel = "dummyLabel"

    # ------------------------------------------------------------------- define all models we want to try
    modelNamesAll = list()
    # modelNamesAll.append({'name': 'vgg16_bn_Mahdi_lbcnn', 'sizeFeatures': 0})
    # modelNamesAll.append({'name': 'ResNet18_lbcnn', 'sizeFeatures': 0})
    modelNamesAll.append({'name': 'ResNet34_lbcnn', 'sizeFeatures': 0})
    ...
    for hierarNum in range(0, 1):
        ...
        for i, (modelData) in enumerate(modelNamesAll):
            ...
            # get current model
            if modelData['name'] == 'vgg16_bn_Mahdi_lbcnn':
                currentModel = vgg16_bn_Mahdi_lbcnn(classesADP[hierarNum]['numClasses'])
                imageSize = 224
            if modelData['name'] == 'ResNet18_lbcnn':
                currentModel = resnet18_lbcnn(pretrained=False, num_classes=classesADP[hierarNum]['numClasses'])
                imageSize = 224
            if modelData['name'] == 'ResNet34_lbcnn':
                currentModel = resnet34_lbcnn(pretrained=False, num_classes=classesADP[hierarNum]['numClasses'])
                imageSize = 224
            ...
            # optim
            optimizer_ft = optim.SGD(filter(lambda param: param.requires_grad, currentModel.parameters()), lr=0.02, momentum=0.9, weight_decay=0.0005)
            ...
            for r in range(0, num_iterations): #(nFolds-1)
                ...
                util.splitDBTrainValTest(indexes, os.path.join(dirOutDatastore, dLabel), dirOutTrainTest,
                ...
                util.print_pers("time train: {0}".format(time_train), fileResultNameFull)
