...
if __name__ == '__main__':
    ...
    # ------------------------------------------------------------------- db info
    dirWorkspace = 'D:/Workspace/DB HEM - Public (test)/ALL_IDB/'
    ...
    modelNamesAll = list()
    # modelNamesAll.append({'name': 'vgg16_bn_Mahdi_lbcnn', 'sizeFeatures': 512})
    # modelNamesAll.append({'name': 'ResNet18_lbcnn', 'sizeFeatures': 512})
    modelNamesAll.append({'name': 'ResNet34_lbcnn', 'sizeFeatures': 512})


    # ------------------------------------------------------------------- loop on hierarchies
    # for hierarNum in range(0, 3):
    for hierarNum in range(1, 2):
        ...
        for i, (modelData) in enumerate(modelNamesAll):
            ...
            for r in range(0, num_iterations): #(nFolds-1)
                ...
                """
                all_params_lbcnn, train_params_lbcnn = count_parameters(currentModel)
                print(all_params_lbcnn, train_params_lbcnn)
                ...
                if not os.path.exists(dirOutTrainTest):
                    os.makedirs(dirOutTrainTest)
                splitfolders.ratio(dirDbTest, output=dirOutTrainTest, seed=random(), ratio=(.4, .1, .5))
                util.print_pers("", fileResultNameFull)
