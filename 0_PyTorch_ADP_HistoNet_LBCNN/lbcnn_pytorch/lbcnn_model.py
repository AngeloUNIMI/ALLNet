...
class BlockLBP_mod(nn.Module):

    def __init__(self, numChannels, numWeights, outPlanes, stride, dilation, sparsity=0.5):
        super().__init__()
        self.batch_norm = nn.BatchNorm2d(numChannels)
        self.conv_lbp = ConvLBP(numChannels, numWeights, kernel_size=3, sparsity=sparsity)
        self.conv_1x1 = nn.Conv2d(numWeights, outPlanes, kernel_size=1)
        self.maxP = nn.MaxPool2d(kernel_size=3, stride=stride, padding=dilation, dilation=dilation)
...
class Lbcnn(nn.Module):
    def __init__(self, nInputPlane=1, numChannels=8, numWeights=16, full=50, depth=2, sparsity=0.5, numClasses=2):
        super().__init__()
        ...
        self.fc1 = nn.Linear(15488, full)
        self.fc2 = nn.Linear(full, numClasses)
